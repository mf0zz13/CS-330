...
{
	// capturing the first movement so that all subsuqent movements can be calcuated 
	// based off the past positions
	if (gFirstMouse)
	{
		gLastX = xMousePos;
		gLastY = yMousePos;
	...
	// calculating the offset values for movieng the camera
...
{
	// If scroll wheel is moved the change in position is passed to camera's ProcessMouseScroll method 
...
void ViewManager::ProcessKeyboardEvents()
{
	...
	if (glfwGetKey(m_pWindow, GLFW_KEY_P) == GLFW_PRESS)
	{
		...
		g_pCamera->Position = glm::vec3(-10.5f,35.0f, 40.0f);
		g_pCamera->Front = glm::vec3(0.0f, -0.5f, -2.0f);
		g_pCamera->Up = glm::vec3(0.0f, 1.0f, 0.0f);
	}

	// change view to orthographic
	if (glfwGetKey(m_pWindow, GLFW_KEY_O) == GLFW_PRESS)
	{
		bOrthographicProjection = true;

		g_pCamera->Position = glm::vec3(-10.5f, 25.0f, 60.0f);
